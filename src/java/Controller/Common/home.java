...
public class home extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Product> listSoleProducts;
        List<Product> listProductsTopDeal;
        ...
        if (categoryFilter != null && !categoryFilter.isEmpty() && request.getParameter("modP") == null) {
            ...
            listSoleProducts = new ProductDAO().getSoleProducts(categoryId);
            listProductsTopDeal = new ProductDAO().getAllProductsTopDeal(categoryId);
            
        } else {
            // Retrieve all products
            productList = productDAO.getAllProducts();
            listSoleProducts = new ProductDAO().getSoleProducts();
            listProductsTopDeal = new ProductDAO().getAllProductsTopDeal();
            
        }
        
       if (categoryFilter != null && !categoryFilter.isEmpty() && request.getParameter("modP") != null) {
            int categoryId = Integer.parseInt(categoryFilter);
            listNewProducts = new ProductDAO().getNewProducts(categoryId);
       } else {
           listNewProducts = new ProductDAO().getNewProducts();
       }
           
        // Set the product list as a request attribute
        request.setAttribute("listProducts", productList);
        request.setAttribute("listProductsTopDeal", new ProductDAO().getAllProductsTopDeal());
